...
- In each package `package.json`, reference catalog entries with values such as `"@aws-sdk/client-s3": "catalog:"` instead of writing concrete versions.
...
```jsonc
// packages/trpc/package.json excerpt
{
  "dependencies": {
...
- TypeScript, the Next.js and React runtime, SST, AWS SDK, and authentication library majors are grouped into separate stack-specific PRs. Do not combine those migrations with unrelated dependency updates.
...
Automerge still depends on CI being present and meaningful. Keep lint, format, typecheck, unit tests, storage-contract checks, and E2E jobs enabled for dependency PRs. Security alerts can require a faster manual update than the routine schedule.
...
| `pnpm dev:sst`                    | Start `web` and `api` in SST mode.                |
...
| `pnpm storage:*` / `pnpm sst:ws`  | Local object storage and SST workspace helpers.   |
