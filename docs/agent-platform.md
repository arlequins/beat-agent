...
채팅 스트림은 재시도 안전성을 위해 클라이언트 생성 idempotency key를 함께 받는다.
사용자 메시지와 완료된 답변에는 이 키의 fingerprint가 남으며, 네트워크가 응답을
잃은 뒤 같은 요청을 다시 보내도 같은 메시지를 반환한다. 키를 다른 질문에 재사용하면
S3 조건부 충돌로 실패한다. 이는 브라우저 새로고침이나 모바일 재연결이 대화를 두 번
저장하는 것을 막기 위한 것이며, 원본 이벤트와 state를 삭제하거나 덮어쓰지 않는다.
